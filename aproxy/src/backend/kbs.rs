...
impl AttestationProtocol for KbsProtocol {
    ...
    /// components of the TEE key, and an ephemeral nonce provided in the KBS challenge that is
    /// fetched from the server's /auth endpoint. These must be hased in order.
    ///
    /// Make this request to /auth, gather the ephemeral nonce, and return this in the negotiation
    ...
    fn negotiation(
        ...
        // Fetch challenge containing ephemeral nonce from the KBS /auth endpoint.
        let http_resp = http
            .cli
        ...
        // Ephemeral nonce is a base64-encoded byte vector. Inform SVSM of this so it could
        // decode the bytes and hash them into the TEE evidence.
        let params = vec![
            NegotiationParam::TeeKeyPublicComponents,
            NegotiationParam::Base64Bytes(challenge.nonce),
        ];
